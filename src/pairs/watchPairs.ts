...
export const getLimitOrderPairs = async (chainId: number): Promise<IWatchPair[]> => {
  ...
  await Promise.all(
    limitOrderPairs.map(async ([token0Symbol, token1Symbol]) => {
  ...
  const pairs = watchPairs.filter((pair, i) => {
    if (!poolBalances[i].token0.gt('0'))
      throw new Error(`Tokens aren't part of a pool: ${pair.token0.symbol} ${pair.token1.symbol}`)
    return true
  })
